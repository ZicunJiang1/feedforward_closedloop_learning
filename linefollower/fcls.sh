...
usage()
{
    ...
    echo "  $0 0.0001 0.001 42 3,3,6 /tmp/fcl_results"
}
...
BASE_OUTPUT_DIR="${5:-$SCRIPT_DIR/StatDataFcl}"

# Check the executable.
if [[ ! -x "$BINARY" ]]; then
    echo "Error: FCL executable not found:" >&2
    echo "  $BINARY" >&2
...
'; do
    ...
    # Run from build/linefollower so loop.png and copied resources can
    # still be found through their existing relative paths.
    #
